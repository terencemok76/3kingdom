...
function buildGround() {
    ...
    // The official road is maintained through the important Luo River ford.
    for (const x of [11, 12, 13]) for (let y = 12; y <= 16; y++) set(x, y, 0x10006);
    ...
    for (let y = 5; y < height; y++) {
        ...
        set(x, y, 0x10005);
        if (y > 16) set(Math.min(width - 1, x + 1), y, 0x10005);
    }
    for (let x = 15; x <= 23; x++) set(x, 8, 0x10005);
    return cells;
}
...
function addRiverEffectLayer(scene, tileData) {
    ...
    // Strip every prior copy before inserting one canonical layer. This also repairs scenes
    // generated by an earlier matcher that did not account for the node's type attribute.
