...
### 仍未完成 / 後續可擴充
- `Assassination` 雖然已支援手選目標官員，但尚未加入：
  - `bodyguard / anti-assassination`
  - `loyalty` 額外修正
  - 更細的目標保護規則
- 戰鬥君主死亡目前採「參戰兵力全滅才判定戰死」的基線規則，尚未擴充為重創低機率、保鏢、護衛等版本。
...
- 已完成一輪可行的實機 smoke run：
  - Godot 專案可正常啟動
  - 啟動輸出無 script error
  - 地圖材質可正常載入
  - `Phase 1 bootstrap complete` 正常出現
- 本輪 smoke run 代表：
  - 專案目前至少可進入遊戲主流程
  - 啟動層沒有立即崩潰或缺資源錯誤
- 本輪 smoke run 不代表：
...
## 6.4 2026-05-09 存檔 / 讀檔

### 已完成

- 已支援 `WorldState` 快速存檔 / 快速讀檔。
- 已接 `F5` 快速存檔、`F9` 快速讀檔。
- 已驗證存讀後可保留：
  - `Population`
  - 兵種分配
  ...
  - 城市情報
  - 玩家繼承待選資料
- 讀檔後會重綁地圖與 HUD，避免 UI 持有舊 world 參考。
...
## 6.5 2026-05-09 Option / Save Load UI
...
- 存檔刪除功能
- 重新命名既有描述的獨立流程
- 真正接上專案內專用 `BGM / SFX` bus 設計

## 6.6 2026-05-10 Phase 1.5 完成後續整理

### 已完成

- 外交基礎版：
  - `Alliance / Truce / Gift / Demand / Break Pact`
  - AI 可使用
  - AI 對玩家提案 popup
- 間諜基礎版：
  - `Reconnaissance / Sabotage / Incite / Assassination`
  - 月末結算
  - EXP / rank 成功率加成
  - `Assassination` 可手選目標官員
  - 可刺殺君主並觸發繼承
- 君主死亡 / 繼承基礎版：
  - AI 自動繼承
  - 玩家繼承視窗
...
- 月末城市事件基礎版：
  ...
  - 全城市都會判定並寫日誌
- UI / 資料：
  - `Population` 已顯示在城市資訊與 `View -> Cities`
- 存檔 / 選項 / 音效基礎版：
  - `F5 / F9`
  - `10` 格存檔 + 描述
  - `Option` 視窗
  - `BGM / SFX` 開關與音量
  - 設定可保存與啟動載入
  - `Jade_Gate` BGM、UI click、城池 click 音效已接上
...
- 外交深化：
  ...
  - 更長期外交策略
- 間諜深化：
  - `bodyguard / anti-assassination / loyalty` 修正
  - 抓捕 / 處置 / 潛伏分支
- 勢力滅亡深化：
  - 鄰近勢力自動接收城市
  - 更完整滅亡後政治後果
