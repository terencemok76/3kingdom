...
- 戰鬥若有武將被俘，勝方可處置：
  ...
  - `處決`
  - `下獄`
- 玩家若在戰後取得俘虜，會先跳出即時處置 dialog。
- 玩家若在月底戰鬥中取得俘虜：
  - 不會在戰鬥鏈中途插入處置視窗
  - 會在全部戰鬥結束、進入下月月頭後，再統一跳出俘虜處置 dialog
- 若選擇 `下獄`，後續可在 `Personnel -> 管理俘虜` 中處理。
- `管理俘虜` 目前支援：
  - 依目前選中城市列出該城在押俘虜
  - 重新選擇俘虜
  - 執行 `招降 / 釋放 / 處決`
  ...
  - 開著 dialog 切語言即時刷新文案
- 俘虜 `招降` 不再保證成功：
  - 目前主要依 `與我方關係 / 原有忠誠 / 野心` 決定是否接受
  - 額外承諾的 `金 / 寶物 / 官職` 會提高接受機率
  - 只有招降成功時，才會真的扣金、配發寶物、套用官職
  - 若招降失敗，武將仍維持俘虜狀態，不會自動釋放
- 俘虜不是永久安全關押：
  - 每月會依城防、民忠、俘虜自身能力與野心等因素計算脫逃機率
  - 但「本月底剛捕獲、尚未在月頭完成處置」的待處置俘虜，不會先參與該次脫逃判定
  - 若關押城市已失去控制，俘虜會直接脫離拘禁並轉為自由武將
  - 脫逃事件會寫入 log
- 目前也已補上俘虜在一般操作 UI 的整合：
  - `Move` 可勾選並轉移同城俘虜
  - 俘虜轉移時不會變成一般武將，而是僅改變 `JailedCity`
  - `View officer dialog` 會把俘虜直接併入武將清單，不另開獨立頁籤
  - 俘虜在 `官職` 欄顯示為 `俘虜`
  - 若為勢力清單，俘虜的 `城市` 欄代表目前關押城市
  - 若為本城清單，只顯示目前關押於所選城市的俘虜
- 回歸驗證流程已整理於：
  - `docs/PRISONER_REGRESSION_CHECKLIST_ZH.md`
...
- 補排程列表中的建設點變化提示
- 依 `docs/PRISONER_REGRESSION_CHECKLIST_ZH.md` 完成俘虜管理與脫逃驗證

### 11.2 Should

- 做 faction-level `Technology` 最小版
- 讓 AI 依城市角色選擇建設方向
- 補俘虜管理的更多互動回饋，例如數值提示或更完整 jail 資訊

### 11.3 Nice-to-have

- 將 `Siege` 拆成攻城子類型
- 為建設加入更完整的預估完成時間 UI
- 延伸俘虜系統到忠誠、勸降成功率與勢力關係修正
