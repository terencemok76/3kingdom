...
- `休戰`
- `贈禮`
- AI 對玩家勢力的 `同盟 / 休戰 / 贈禮` 已改為 `提案制`，不再直接自動生效。
...
- `LastUpdatedYear`
- `LastUpdatedMonth`
- 若 AI 對玩家提出 `同盟 / 休戰 / 贈禮`，月底會先顯示提案彈窗，由玩家選擇 `接受 / 拒絕` 後再繼續結算。
- 玩家拒絕時：
- `同盟 / 休戰` 不成立。
- `贈禮` 退回 AI 原城市預留金。
- 目前拒絕不附加額外關係值懲罰。
...
- 若 AI 對玩家勢力提出 `同盟 / 休戰 / 贈禮`，應進入 `玩家可回應提案` 流程：
- AI 先正常排定外交命令。
- 月底若目標為玩家勢力，先暫停流程並顯示提案彈窗。
- 玩家可選擇 `接受` 或 `拒絕`。
- 玩家接受時，依原本外交規則結算。
- 玩家拒絕時，提案不生效；若為 `贈禮`，需返還 AI 原城市預留金。
- 此提案流程目前只涵蓋 `同盟 / 休戰 / 贈禮`。
- `索貢 / 毀約` 尚未納入玩家接受 / 拒絕互動。
- 外交狀態至少應追蹤：
- 關係層級（`敵對 / 中立 / 友好 / 同盟`）
