...
- 設計鎖定日期：`2026-04-28`
- 完成里程碑：`Phase1.5-Completed-v1`
- 完成日期：`2026-05-10`
- 目的：在 Phase 1.5 實作前鎖定系統設計。
- 狀態：Phase 1.5 基線功能已完成並完成文件同步；其餘項目視為 `Phase 1.5+` 擴充或 polish。
- 可在實作期間微調的項目：
- 數值 buff 百分比。
- 經驗值取得量與升階門檻。
- 兵種成本、維持費與相剋倍率。
...
- 弩兵徵募需要 `弓坊`。
- 投石車 / 攻城兵徵募需要 `工坊`。
- `徵兵 / Recruit` UI 應隱藏目前城市尚未滿足條件的兵種選項：
- 未有 `弓坊` 時不顯示 `弩兵`。
- 未有 `工坊` 時不顯示 `投石車 / 攻城兵`。
- 未有馬匹時不顯示 `騎兵`。
...
- 城市設施以等級管理，而非單純布林旗標：
- `弓坊等級 / BowWorkshopLevel`
- `工坊等級 / SiegeWorkshopLevel`
- `馬場等級 / HorsePastureLevel`
- 城市設施同時保留建設進度值：
- `BowWorkshopProgress`
- `SiegeWorkshopProgress`
- `HorsePastureProgress`
- 設施升級改為 `建設點 / Construction Points` 累積制，而非單次建案完工直接升級。
- 目前升級門檻為：
- `Lv0 -> Lv1` 需 `100`
- `Lv1 -> Lv2` 需 `200`
- `Lv2 -> Lv3` 需 `300`
- 其後依 `100 * (目前等級 + 1)` 遞增。
- `馬場等級` 會提高每年 1 月的馬匹繁殖率。
- `工坊等級` 會提高 `投石車 / Siege` 在攻城戰中的城防壓制效率。
...
- 建設。
- 玩家選擇工作、武將與執行月份。
- 玩家同時指定 `每月投入金`；目前語意為「每月成本」，不是一次性總預算。
- 若設定 `3` 個月、`每月投入金 500`，則總成本為 `1500`。
- 每名武將同時只能被派任一項工作。
- 內政每月底結算一次。
- 內政每月底先檢查城市是否足以支付該月成本；足夠才扣款並執行效果。
- 若當月資金不足，排程自動轉為 `Paused`，待後續資金恢復且有可用武將時可自動恢復。
- 完成每月工作後，城市相關能力提升。
- 未來會加入經驗與職階升級。
- `Construction` 在 v1 先細分為：
- `弓坊`
- `工坊`
- `馬場`
- `Construction` 每月底會把 `每月投入金 + 武將能力 + 建設職系加成` 轉為建設點。
- 建設點會累積到對應設施：
- `弓坊`
- `工坊`
- `馬場`
- 當累積值達升級門檻時，提升對應設施等級，超出的建設點保留到下一級。
- 城市主 UI 應顯示 `弓坊 / 工坊 / 馬場` 的 `等級(目前進度/下級門檻)` 摘要。
- `View Cities` 應顯示相同格式的設施進度欄位，方便跨城比較。
- 內政排程列表應顯示建設項目、剩餘月數、每月投入金與目前狀態。
...
- 徵兵需要指派一名可用武將。
- 玩家先選擇兵種，再輸入徵兵數量。
- UI 應顯示依下列條件計算出的 `可徵募上限`：
- 城市 `Gold`
- 城市 `Food`
- 城市 `Population`
- 特殊條件資源，例如騎兵所需 `Horses`
- 兵種解鎖條件，例如 `弓坊 / 工坊`
- 玩家輸入的徵兵數量不可超過目前可徵募上限。
- UI 應即時顯示實際消耗：
- `Gold`
- `Food`
- 若為 `騎兵`，另外顯示 `Horse`
- 指令確認時立刻扣除 `Gold / Food`，騎兵的 `Horse` 於月底依實際成軍數量消耗。
- 徵兵實際成果於月底結算，結果受：
- 指派武將 `Charm`
- 城市 `Loyalty`
- 隨機值
...
- 同一名武將不能同時做多個內政工作。
- 多月排程每月底正確扣月份。
- 多月排程每月底依 `每月投入金` 扣款，而非建立時一次扣完整筆預算。
- 資金不足時排程會自動 `Paused`，資金恢復後可自動恢復。
- UI 可中止排程。
- UI 可顯示建設子項目、每月投入金與目前排程狀態。
- 城市屬性提升正確。
- `建設` 應累積為設施進度值，並在 UI 顯示 `等級(進度/門檻)`。
...
- 商人可購買馬匹。
- 移動指令可轉移馬匹。
- `Recruit` 視窗會先依城市條件過濾兵種，再依玩家輸入數量即時計算成本與可徵募上限。
...
- `內政 / Internal Affairs` 已擴充為 `每月投入金` 模式，月底逐月扣款；若當月金不足會自動暫停，後續可自動恢復。
- `Construction` 已可指定 `弓坊 / 工坊 / 馬場`，並在完工後提升對應設施等級。
...
- `SiegeWorkshopProgress`
- `HorsePastureProgress`
- `弩兵` 招募需求為 `BowWorkshopLevel >= 1`。
- `投石車 / 攻城兵` 招募需求為 `SiegeWorkshopLevel >= 1`。
- `徵兵 / Recruit` UI 會直接隱藏不符合條件的兵種選項。
- `徵兵 / Recruit` 改為數量制成本模型，不再使用單次固定票價。
- `HorsePastureLevel` 會提高每年 1 月馬匹繁殖率。
- `SiegeWorkshopLevel` 會提高 `Siege` 對守城 `Defense` 的壓制效果。
- 城市主 UI 與 `View Cities` 目前都顯示設施格式：
- `等級(目前進度/下級門檻)`
...
- 月末事件日誌已可顯示事件造成的：
  - `Population` 損失
  - `Troops` 損失
## 18.4 2026-05-09 存檔 / 讀檔

### 18.4.1 目前支援範圍

- 已支援完整 `WorldState` 存檔 / 讀檔。
- 目前包含：
  - 年月、亂數種子、God Mode 狀態
  - 城市資料
  - 武將資料
...
- `F9`：快速讀檔
- 快速存檔路徑：
  - `user://saves/quicksave.json`

### 18.4.3 讀檔後處理

- 讀檔成功後會：
  - 重新綁定 `TurnManager.World`
  - 重新綁定地圖資料
  - 刷新 HUD 文字與城市資訊
  - 重新選取玩家可控制城市
  - 清掉暫時性彈窗與月末 pending UI 狀態

### 18.4.4 目前限制

- 目前只有快速存檔 / 快速讀檔，尚未提供正式的存檔列表 UI。
- 日誌內容目前不會隨存檔一起還原；讀檔後會從當前 HUD 狀態重新顯示。

## 18.5 2026-05-09 選項 / 10 格存檔 UI
...
- 預設 BGM 使用 `assets/bgm/bgm_main_menu_01.ogg`。
- 第二首 BGM 使用 `assets/bgm/bgm_main_menu_02.ogg`。
- 進遊戲後先播 `bgm_main_menu_01.ogg`，再接 `bgm_main_menu_02.ogg`，之後輪播。
...
- 支援 `10` 個存檔槽位。
- 每個槽位使用獨立檔案：
  - `user://saves/slot01.json`
  - `user://saves/slot02.json`
  - ...
  - `user://saves/slot10.json`
- 玩家可輸入每格的 `description`。
- 視窗會顯示：
  - 槽位是否為空
  - 描述
  - 劇本名稱
...
- 存檔格式已升級為包含：
  ...
  - `SavedAtUtc`
  - `World`
- 讀檔仍相容舊版僅存 `WorldState` 的存檔。

## 20. Phase 2 起始：中央任命與建議
...
- 同一勢力同時間只能有：
  ...
  - `1` 位首席軍師
- 目前由玩家手動任命。
- 任命入口位於左側城市指令列新增的 `Advisor / 參謀` 按鈕。
- 任命限制：
  - 只能任命本城武將
  - 君主不可直接兼任這兩個中央職位
...
- 若中央職位持有人：
  ...
  - 被開除
  - 被登用到其他勢力
- 對應中央職位會自動清空，避免保留失效指派。
- 若中央職位持有人之後繼位成君主，也會自動清空原中央職位。
