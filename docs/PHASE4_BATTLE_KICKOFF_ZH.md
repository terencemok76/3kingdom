...
- 智略至少 `70` 的 AI 武將若站在道路上、確實令尚未行動的同隊一般戰隊無法取得完整接敵路徑，且可安全移往相鄰非道路格，會先讓出道路；只會讓位給同隊、可因該格騰空而重新取得 A* 路徑的戰隊，不會穿越或取代任何友軍／敵軍佔格。讓位後下一個 AI step 會重新規劃被阻擋戰隊的正常行動。
- 工兵的新建橋梁計畫會以路徑縮短幅度、工期、接近成本、智略及威脅評分；除了野戰河流，也會在 Moat Siege 的壕溝評估可連接己方與敵軍／目標的壕溝走廊。既有木橋受損仍可作為修復／續建目標；工程決策與移往施工位的紀錄均本地化。
...
- 攻城戰所有城門均關閉且未破壞時，城外 L0 的攻方弓兵、弩兵與投石車不能鎖定或傷害城內 L0 守軍；可攻擊關閉城門本體，亦可攻擊城牆 L2 的守軍。任一城門開啟或破壞後，城外至城內 L0 的遠距攻擊恢復合法；AI 使用相同攻擊合法性。
...
- 攻方 AI 會另行比較完整城門的突破候選：可合法攻擊城門的衝車以結構傷害、預估破門回合與自身受威脅程度評分，並優先移至合法攻門格；一般戰隊只在預估兩擊內能收尾時才攻門，避免以低結構傷害取代衝車。雲梯仍維持登牆／奪門的平行入口；一旦攻方佔領城門 `L0`，會以既有開門行動讓地面主力突入。
- 同一座城門組首次耐久歸零時，戰場中央會以非模態通知顯示「城門 `(x, y, L0)` 已被攻破」；若破門來源是有武將的一般戰隊，會同步觸發資料驅動的 `gate_breach` 台詞，優先度高於一般攻擊台詞，且不受同武將一般發言冷卻限制。火焰攻破沒有可歸屬的武將時，仍顯示通知但不顯示台詞。
