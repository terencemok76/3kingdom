...
- 非遮擋單位（例如牆頂武將）移動時，不會清除其他仍受城門／城牆前景遮擋的地面單位剪影；只有移動起點或終點本身受遮擋時，才暫時揭露剪影以播放該單位的移動動畫，完成後立即重建。移動中的原始 marker 在 tween 結束前會暫時保留可見，避免資料已先更新為受遮擋終點格時被 visibility refresh 隱藏而造成由起點直接跳至終點。
...
- 關閉且未破壞的城門 `L0` 上若有守軍，正常單位立繪會被門樓前景遮住，只顯示低透明度的陣營陰影、名字牌與兵力條；陰影會向相鄰內城 courtyard 偏移，表達其在內城側駐守而非站在城外拱門前。一般可見性刷新亦不得重新顯示被遮蔽的原始 marker，確保同一名武將不會同時顯示為兩個人物。城門開啟或破壞後恢復正常立繪。此為純渲染規則，不改變城門控制、移動或攻擊合法性。
...
- 戰鬥畫面以 locale 顯示勢力名稱：內部仍以 `Team A / Attacker`、`Team B / Defender` 供戰鬥規則與 AI 判斷，但 top bar、行動方提示、HUD 與武將發言框顯示「曹操軍／董卓軍」（英文為 `Cao Cao`／`Dong Zhuo`）。top bar 的戰況時間線格式為「`191 年 04 月｜本月戰日：4 / 10｜早晨｜回合：15`」：campaign 使用目前月已完成戰日加一，獨立測試戰場使用目前日期日數；兩者均以每月 `10` 戰日為上限。發言框順序為勢力名、武將名、台詞。
...
- 戰鬥開始會先聚焦攻方發言武將並播放開戰台詞，再聚焦守方發言武將並播放開戰台詞；兩段發言結束後才在畫面中央顯示目前勢力的本地化回合橫幅（如「曹操軍回合／董卓軍回合」）。其後每次行動方切換仍立即顯示橫幅。橫幅約 `2.5` 秒後自動關閉，且不攔截滑鼠與操作。
...
- 每筆同盟請求於確認守城後，會先寫入 Gameplay Log，並以「同盟援軍回覆」視窗顯示同意或拒絕；所有回覆確認後才切換至戰場。盟軍正式援軍依請求規模選擇 `1`／`2`／`3` 名 Leadership＋Combat 較高、再以 Intelligence 排序的盟軍武將，避免把所有可用武將切成大量零碎小隊；使者借調步兵與這些 AI 部隊合計不超過請求上限。
...
- 援軍只在己方時段開始前的 Dawn 進場。攻方只使用攻方外側起始部署區；守方援軍也只使用守方外側起始部署區，不能直接生成於內城。只有原本的城市守軍可在內城開始。入口受阻時進入等待隊列，不可重疊生成或直接出現在敵軍旁邊；中央事件提示與戰鬥紀錄會列出抵達援軍的勢力、武將／兵種／兵力及 Gold／Food。援軍實際部署後，戰鬥紀錄另以「原有 + 援軍 = 最終」列出該側 Gold／Food，且 top bar 立即同步最終值。若當前有攻守方回合 banner，援軍中央提示會等 banner 隱藏後才顯示，避免重疊；同一 Dawn 的多筆援軍會依勢力合併為一則提示，避免重複標題堆疊。
...
4. **已完成—snapshot 與場景切換主幹**：第十個完整作戰日後將戰場 DTO 寫入 Campaign、返回 Gameplay，下一月可由「持續戰役」重載；Field → City 使用同一 Campaign 並清除不相容地圖 snapshot。
   ...
   - 攻擊與守城配兵的每位武將列均提供 locale 化的「最大／Max」按鈕。按下後只填入該兵種扣除其他已配武將後的剩餘兵力；切換兵種或修改數量會立即更新其他列的可填上限，避免同一兵種重複配置。兵種選單保留彈性寬度，但以固定最小寬度搭配緊湊的最大按鈕，減少畫面浪費；最大按鈕沿用確認指令按鈕主題。攻擊目標選單以「【勢力】城市」顯示，守城時亦會標示來襲勢力，避免只看城市名稱而無法辨認敵軍歸屬。
...
11. **已完成—戰後俘虜、繼承與復國**：Campaign 奪城俘虜會與戰報回到 Gameplay；玩家勝方以既有俘虜面板選擇招募、釋放、監禁或處決，AI 勝方則依能力與忠誠自動選擇四種處置。釋放時優先回到被俘前的 `HomeCityId`，以在野身分停留。因無繼承人而亡國的被俘君主會保留原勢力 ID；後續在野移動進入無主城市時會立即佔城、恢復君主與原勢力。君主死亡時 AI 自動選繼承者、玩家要求選擇；無繼承人時勢力城市轉無主。玩家除失去所有城市外，若仍有城市但所屬武將全數死亡，也會敗北。

12. **已完成—勢力結果提示**：Gameplay 偵測到原本持有城市的勢力失去全部城市時，會在既有日誌之外以同戰報風格的 modal 顯示勢力滅亡；日誌以現任君主顯示為 `勢力滅亡：【君主】軍`，使繼承後改名的勢力來源仍容易辨認。多個勢力會依序顯示。玩家因失去全部城市或沒有存活的所屬武將而敗北時，顯示「遊戲敗北」modal；確認後隱藏地圖與 Gameplay HUD、恢復 Main Menu 與主選單音樂。若戰報仍在畫面上，結果提示會等戰報關閉後才顯示。
...
14. **已完成—玩家戰役撤退目的地對話框**：玩家帶武將戰隊抵達撤離區後，會以 `RetreatDestinationDialog.tscn` 顯示有效的來源己方城市及相鄰己方／無主城市；來源城市永遠優先列出為「返回來源城：城市」，每個其餘候選都是明確的「撤往：城市」按鈕，而非不可辨識的文字。此候選集合同時由 `BattleCampaignService` 驗證與執行，AI 也使用相同規則。對話框沿用主遊戲深色面板、金棕標題列與指令按鈕樣式，初次置中、可拖曳並攔截戰場輸入。沒有合法目的地時不開啟空白選單，直接顯示「沒有可前往的城市，無法撤退。」
