...
- 戰略地圖城市名稱標籤已相對城市圖示上移 10 像素，以減少與圖示底部重疊。
- 戰略地圖的顧問對話框已支援武將 `advice_profile` 視角：共用戰況結論會保留，但不同顧問會依其長策、權變、經略、交涉、果決或審慎取向說明取捨；詳見 `docs/ADVISOR_PROFILES_ZH.md`。
- 俘虜管理的招降不再顯示或計算俘虜的舊勢力忠誠；招降機率改由關係、君主魅力、贈禮、職位與野心決定，成功後以新勢力忠誠 60 加入。

## 3. Phase 4 目標

- 讓戰鬥從目前可運作基線版，提升到更清楚、更可調、更有策略差異的系統。
- 本階段以「功能完整、規則一致、可讀與可重現測試」為結案標準；不以最終數值平衡為結案條件。
- 優先處理：
  - 戰鬥流程可讀性
  - 兵種差異
  - 攻城器與城防的戰鬥意義
...
- Phase 4 只做防止規則失效的安全調整：不得有必勝／無效的明顯選項、無限循環、AI 長期卡死，或玩家與 AI 使用不同結算規則。
- 兵種、攻城器、地形、城防、援軍、補給與傷兵恢復的最終數值平衡，不在 Phase 4 收尾時定案。
- 完整平衡必須待 Phase 5 將城市特色、資源鏈、武將性格與月末結果回顧納入同一戰略循環後，再以獨立的 Balance Phase 處理。
...
- 大規模戰場演出或動畫化戰鬥表現
- 最終戰鬥數值平衡與長期戰略勝率調整（移交至 Phase 5 整合後的獨立 Balance Phase）

註：
- 城門 / 城牆耐久已進入 prototype 基線，不再列為完全未開始項目。
- 受損來源、破壞演出與完整 AI 目標選擇仍應完成；最終數值平衡不阻擋 Phase 4 結案，將移交至後續獨立 Balance Phase。

## 6. 建議切入順序

1. 先完成所有戰鬥流程的結算結構與參數責任。
2. 確認 troop / siege / defense 的規則語意、限制與 UI 呈現一致。
3. 完成 `Attack` UI、結果顯示與戰役結果回寫。
4. 補足 AI 的部署、出兵與目標選擇，並以固定情境重播確認不會卡死或違反共用規則。
5. 結案後進入 Phase 5 的城市／人物／月末整合；完整數值平衡留待整合後的獨立 Balance Phase。
...
- 玩家可理解戰鬥勝負、傷亡、撤退、俘虜與戰役結果為何發生。
- 兵種、攻城器、地形與城防已各有可操作、可讀的規則差異。
- 野戰、攻城、援軍、撤退、跨月續戰與戰報可完整走通並正確回寫大地圖。
- 玩家與 AI 共用同一套合法性與結算規則；固定測試情境中 AI 不會長期卡死、違反規則或持續做出明顯無效行動。
- 最終戰鬥數值平衡不屬於本完成判定，待 Phase 5 整合城市／資源／人物／月末循環後，以獨立 Balance Phase 進行。
...
- 城門通行與開關目前採 prototype 規則：
  ...
  - 已破壞城門會保持可通行狀態
  - 關閉城門會阻擋外側地面進入或離開城門地面層；守方仍可由內城庭院移入城門地面層 `L0` 駐守、開關門或登上牆頂
  - 城內地面不可直接踏上相鄰城牆／塔樓的牆頂層 `L2`；守軍必須先移至城門地面層，再使用同座標的城門垂直通道登上牆頂
...
- 補給車僅在相鄰友軍有重傷、士氣不高於 `30` 或受損投石車時，優先於一般戰隊行動；一般補給、補充武器及移動後補給會以回復量、士氣缺口、修理量、缺彈程度及路徑成本計分，再與攻擊、工程與堡壘候選共同排序。彈藥為 `0` 的兵器會取得額外補彈分數，投石車再取得攻城器加分；補給車可跨回合接近遠處缺彈友軍。無彈投石車若沒有相鄰弱近戰目標，會向己方補給車撤回或等待，不參與 Fortress／一般接敵移動；純移動時一般武將戰隊優先於攻城器，且投石車不會為同距或後退格來回移動，避免阻塞橋頭。
...
- 一般追敵移動會先對每個可見敵軍的合法鄰近接敵格計算完整 A* 路徑，依實際 Energy 成本與步數選擇目標，再沿該路徑前進至本回合可達的最遠安全格；河流、橋梁、城門與阻擋物因此會納入追擊路線，而不再只按敵軍的直線距離移動。若受友軍堵路或地形封閉而無完整路徑，才退回既有的本回合局部合法移動。
- 新開啟的 standalone battle 預設為本月戰日 `1 / 10`；campaign battle 則以當月已完成戰日加一顯示，續戰時不會重設為第一日。
- 城門或城牆前景遮擋的單位以靜止剪影呈現；剪影一律使用該單位目前面向的 idle scene，不複製攻擊、受傷或移動中的暫態動畫，避免遮擋重新整理後剪影持續播放攻擊／受傷動作。
- 任一受遮擋單位移動時，只會移除正在移動者起點的靜態剪影；其他靜止的城門／城牆前景遮擋剪影會持續顯示。只有移動起點或終點本身受遮擋時，才暫時揭露該單位以播放移動動畫，完成後立即重建。移動中的原始 marker 在 tween 結束前會暫時保留可見，避免資料已先更新為受遮擋終點格時被 visibility refresh 隱藏而造成由起點直接跳至終點。
- 智略至少 `70` 的 AI 武將若站在道路上、確實令尚未行動的同隊一般戰隊無法取得完整接敵路徑，且可安全移往相鄰非道路、非橋梁格，會先讓出道路；只會讓位給同隊、可因該格騰空而重新取得 A* 路徑的戰隊。驗證路線時會把讓位終點視為新佔用格，因此不能把原道路阻擋移到橋梁、單一路口或同一條必經路線；衝車通往完整城門的 A* 路徑，以及已部署雲梯的地面攀登落點，亦會保留為攻城通道，不能成為讓位終點。不會穿越或取代任何友軍／敵軍佔格。AI 先檢查可一擊破門的決定性攻門；若不能立即破門，則道路讓位會先於一般攻擊與移動後攻擊，避免武將以普通攻門傷害耗盡本回合而持續堵住雲梯、城門或破口的推進路線。讓位後下一個 AI step 會重新規劃被阻擋戰隊的正常行動。
- 工兵、糧車、投石車、雲梯與衝車沒有個人武將時，會在 `8` 格指揮半徑內取同隊、未混亂、仍在場武將的最高智略作為「指揮智略」；沒有合格武將時固定使用基礎值 `50`。這個值只調整工程、補給、投石與攻城器的風險／優先評分，不能改寫合法路徑，也不會令器械追隨武將位置。雲梯與衝車仍先以牆頂入口／城門、附近友軍跟進、守軍與威脅決定攻城目的地。
- 工兵的新建橋梁計畫會以路徑縮短幅度、工期、接近成本、智略及威脅評分；除了野戰河流，也會在 Moat Siege 的壕溝評估可連接己方與敵軍／目標的壕溝走廊。若新橋與既有可通行橋相鄰、至少兩支同隊非工兵部隊在該 crossing 周邊壅塞，且新橋完成後仍可通往同一目標，會作為平行分流橋候選，即使最短步數不縮短；不存在上述壅塞時仍須至少縮短 `3` 格。既有木橋受損仍可作為修復／續建目標；工程決策與移往施工位的紀錄均本地化。
- 步兵、槍兵與近衛兵可花費 `5` Energy 對相鄰受損橋梁做緊急修復（`150` 耐久），也可對相鄰、未破且受損的守方城門做緊急修復（`250` 耐久）；工兵維持較高的正式工程效能，分別修復橋梁 `450`、城門 `600` 耐久。修城門會關門；若仍有同隊地面部隊在城外，玩家指令與 AI 都不得修門，以免把友軍關在外面。守方 AI 會把受威脅或低於半耐久的城門維修列為候選，並優先於一般橋梁維修。
- AI 的所有 `Decision:` 紀錄（雲梯部署與留守、補給、守備、撲火、修橋、堡壘、攻擊、隱蔽、無彈投石車與移動後攻擊）均透過 `log.ai.*` locale keys 顯示；數值、座標、分數與變異維持可檢閱。
...
- 所有獨立戰場測試（野戰及攻城戰）與由 gameplay 進入的戰鬥 top bar 都有 `偵錯 / Debug`，會開啟非模態、可由標題列拖曳的 Debug 浮動面板；不顯示暗幕，也不攔截面板外的戰場輸入。獨立測試戰場預設攻守雙方均為 AI 單步檢閱；Debug 面板可使用「啟用／停用 AI、開始回合、下一步、攻方／守方糧食：1 日、本月上限：1 日」，而時間、天氣、風向與風力按鈕保留在 top bar 並可直接調整。campaign battle 未開始時維持原有正式回合流程；按下「開始回合」後會與獨立測試相同，將攻守雙方都設為 AI 單步檢閱，因此目前行動方的「下一步」可立即使用。Debug 回合進行中仍可對目前行動方切換 AI／玩家控制；停用 AI 後可手動接管尚未行動的隊伍，再啟用 AI 則可繼續用「下一步」測試。兩個一日糧食按鈕會將指定方 Food 設為其目前 active troops 的一日消耗，方便驗證低糧 AI 行為，不修改 Scenario 初始資源。「本月上限：1 日」在任何 Debug 戰場均可按下，將上限設為目前日數；下一個完整戰鬥日結束即顯示本月暫停結果。戰役會保存 snapshot、提示下月續戰後返回 Gameplay；獨立測試會保留月末結果，提示重新開始測試即可模擬下月續戰。它不改變一般每月 10 日規則。一日分為 `Dawn / Morning / Afternoon /Night` 四個時段；攻、守雙方各完成一次行動後才結束一個時段。每個時段扣除當日 Gold／Food 消耗的四分之一，整數餘數會帶到後續時段，避免四次取整令實際消耗增加；只有 `Night -> Dawn` 才讓日期前進，結算整日低糧／斷糧士氣與逃兵效果。
...
- battle team 可使用 prototype `Retreat`，但必須先走入己方撤離區才可離開戰場：攻方預設為 SW、守方預設為 NE 的 `2×4` 格；每張 `BattleScenarioDefinition` 可覆寫雙方撤離格。兩方撤離區會常駐顯示：攻方紅色、守方藍色；未抵達前 Retreat 按鈕會停用並顯示提示。AI 的危急撤退會先朝可到達的撤離區推進，抵達後才離場。獨立測試戰場維持只移除戰場單位的既有行為；Campaign 中，有武將的隊伍抵達出口後，守方以目標城市、攻方以出發城市為錨點，僅可選擇相鄰的己方或無主城市。玩家會看到選城對話框；AI 優先撤往駐軍較強的己方城市，沒有己城才會進入無主城。進入無主城時立即轉為該隊伍勢力；若沒有合法目的地，保留在戰場並顯示「沒有可前往的城市」。沒有武將的 Worker、補給車與攻城器不選城，會回到隊伍自己的 `OriginCityId`；來源城市不存在或已失去同勢力控制時同樣不能撤退。成功的 Campaign 撤退會立即寫回目的地，並記錄 `RetreatDestinationCityId`，所以戰役結算不會重複歸還部隊、器械或武將。成功撤退後，該隊伍會從目前格子與畫面移除，並釋放佔用格；top bar 的對應陣營總兵力會扣除該隊伍撤退時的剩餘 `TroopCount`。無論玩家或 AI 觸發撤退，畫面中央皆會顯示單位名稱及 `Retreat` 的提示框約 2 秒後自動消失；武將隊顯示「武將名／兵種」，Worker 等無武將隊伍顯示本地化兵種名。
- 有武將的戰隊撤退時，原撤退武將仍先播放既有 `retreat` 台詞；其台詞結束後有 `55%` 機率播放本地化的 `enemy_retreat` 回應台詞：優先從撤退位置 `6` 格內的可見敵方武將隨機選擇；若附近沒有，則從戰場上任一可見敵方武將隨機選擇。這不影響撤退結算，重要高優先台詞仍可覆蓋此回應。
- 攻城戰所有城門均關閉且未破壞時，城外 L0 的攻方弓兵、弩兵與投石車不能鎖定或傷害城內 L0 守軍；可攻擊關閉城門本體，亦可攻擊城牆 L2 的守軍。攻擊關閉城門只使用結構傷害，不會依同格守軍計算兵種相剋或在 Battle Log 顯示兵種相剋。關閉且未破壞的城門 L0 即使被火計點燃，仍只承受城門耐久傷害；同格守軍不會承受火計初始、持續火勢或進入火格傷害。燃燒中的完整關門格亦是火勢終點，風向不能使火勢由它傳入或傳出內城；開門或破門後，才恢復一般火勢傳播與該 L0 守軍的火傷。任一城門開啟或破壞後，城外至城內 L0 的遠距攻擊恢復合法；AI 使用相同攻擊合法性。
- 城門開啟或破壞後，攻方衝車與雲梯不可進入城內 L0 courtyard：衝車由 AI 優先移往攻方後方以清出城門通道；雲梯維持在城外牆邊作為登牆支援，只有佔住城門格時才向後撤離。後方被友軍佔據而沒有合法路線時，攻城器會原地待命，不會穿過或推開己方武將；破門後的雲梯留守、攻城器後撤與無路留守，以及一般破門攻擊的兵種名稱均透過 `log.ai.*` locale keys 顯示。
- 攻城 AI 的雲梯車會先建立「靠牆架梯」任務：只選擇可同時接到外側地面與牆頂 `L2` 的非城門格，以完整 A* 路線成本、空置牆頂入口數及附近可攀登的攻方步兵／槍兵／弓兵數量評分。抵達後雲梯固定在原地，讓既有跨層移動規則把己方部隊帶上牆頂；不會再以一般追敵邏輯把雲梯開離可用牆段。留守紀錄使用 `log.ai.ladder_hold` locale key 顯示。
- 攻方 AI 會另行比較完整城門的突破候選：可合法攻擊城門的衝車以結構傷害、預估破門回合與自身受威脅程度評分，並優先移至合法攻門格；一般戰隊只在預估兩擊內能收尾時才攻門，避免以低結構傷害取代衝車。關閉城門格上的守軍不會被一般攻擊、合擊或移動後攻擊誤當作目標；只有破門專用候選可攻擊城門。若守方尚有未行動、可合法修門的部隊，且其下回合維修量足以完全抵銷本次非收尾攻門傷害，該攻門候選會被拒絕，避免弓兵等單位無限耗彈。雲梯仍維持登牆／奪門的平行入口；一旦攻方佔領城門 `L0`，會以既有開門行動讓地面主力突入。
- 城門每次承受一般攻擊或持續火勢的耐久傷害，都會新增本地化 Battle Log 記錄，顯示來源、目標格、實際傷害與剩餘耐久；因此「攻擊城門」不再只留下出手紀錄而看不到耐久變化。完成 `Work` 後的 Action 記錄亦會依實際目標顯示修建／修復橋梁、修復城門或設置／拆除木柵，不能把城門工程誤記為橋梁工程。同一座城門組首次耐久歸零時，戰場中央會以非模態通知顯示「城門 `(x, y, L0)` 已被攻破」；若破門來源是有武將的一般戰隊，會同步觸發資料驅動的 `gate_breach` 台詞，優先度高於一般攻擊台詞，且不受同武將一般發言冷卻限制。火焰攻破沒有可歸屬的武將時，仍顯示通知但不顯示台詞。
- 城門開關改為玩家與 AI 共用的 `ToggleGate` action：一般戰隊必須實際站在未破壞的城門地面層 `L0` 才能切換。攻方 AI 已控制關閉的城門時會開門供地面主力進攻；不能隔牆、隔格或由攻城器開門。
- 守方 AI 的 `Gate Defense` 優先順序為：先反擊或回防攻方已佔的城門，其次關閉己方控制的開門；若攻方位於城門／相鄰外側格／該城門牆頂，或雲梯已提供相鄰牆頂入口，守方會把城門列為高優先移動與 Guard 目標。守門優先度同時估算衝車／可收尾戰隊到達攻門格的 A* 能量回合、城門現有耐久與所需破門攻擊回合，因此接近破門的城門會比單純鄰近敵軍優先。位於受威脅城門牆頂的守軍會先保留直接攻擊，沒有合法目標才 Guard 固守；城內隊伍則保留既有的合法攻擊與生存／掩體決策。守方只有在外側僅一支攻方、城門牆頂未被攻方控制、兩支以上一般戰隊在兩格內支援、門外有可踏出的空格，且沒有衝車／收尾戰隊可在短期破門時，才建立跨回合 `SortiePlan`：本回合開門、下個守方回合由守門隊真正走到城外格並攻擊指定孤立敵軍。出城隊兵力低於出發時 35%、低士氣、遭兩支相鄰攻方包圍、目標消失，或城門即將被破時，會優先穿過開門回到內城；支援守軍接手站上城門後關門。雙格城門在 `SortiePlan` 期間被視為同一座門，任一格建立的開門計畫都會保留整個門組，不能由另一格的普通關門邏輯提前關閉。計畫持續期間不會被一般關門邏輯立即取消。守方工兵位於內城時，不能因一般追敵或移動後攻擊評估而穿過已開城門到城外；只有明確的工程或 `SortiePlan` 任務才可另行授權出城。其餘狀況不主動開門。
- 關閉且未破壞的城門 `L0` 上若有守軍，正常單位立繪會被門樓前景遮住，只顯示低透明度的陣營陰影、名字牌與兵力條；陰影直接對齊該單位的邏輯城門格中心，不另行向內城偏移。一般可見性刷新亦不得重新顯示被遮蔽的原始 marker，確保同一名武將不會同時顯示為兩個人物。城門開啟或破壞後恢復正常立繪。此為純渲染規則，不改變城門控制、移動或攻擊合法性。
- 受門樓／前景遮擋的單位受傷時，會先暫時顯示正常受傷動畫；動畫完成並切回 idle 後才重建其 silhouette，避免以受傷 sprite 複製出永久動畫陰影。
- 武將在城門 `L0` 執行開門或關門時，會透過既有非模態、資料驅動的武將發言框顯示 persona 對應台詞；玩家與 AI 使用相同 `ToggleGate` executor，因此兩者皆可觸發。開門與關門事件不受一般同武將發言冷卻抑制，以確保守門／出城反擊的關鍵狀態變化能被看見。
...
- 戰鬥畫面以 locale 顯示勢力名稱：內部仍以 `Team A / Attacker`、`Team B / Defender` 供戰鬥規則與 AI 判斷；頂欄兩支勢力資訊固定以「攻方：君主名」與「守方：君主名」顯示，不使用 `【】` 包裝。行動方提示、HUD 與武將發言框同樣使用君主名（英文為 ruler name）。top bar 的戰況時間線格式為「`191 年 04 月｜本月戰日：4 / 10｜早晨｜回合：15`」：campaign 使用目前月已完成戰日加一，獨立測試戰場使用目前日期日數；兩者均以每月 `10` 戰日為上限。發言框順序為勢力名、武將名、台詞。
...
- 游標位於 `Unit Command` 面板時，滑鼠滾輪事件會由面板攔截，不會縮放地圖；游標在地圖範圍時仍可用滾輪縮放。
- 點選有武將的一般戰隊時，`Unit Command` menu 會從既有角色肖像圖集裁切並顯示該武將肖像，以及目前戰鬥實際採用的 `Intelligence`／`Combat` 屬性；目前野戰的夏侯淵、張郃、董卓、李傕、郭汜、曹洪與曹純均有對應肖像。繁中介面顯示中文姓名與「智略／戰鬥」，英文介面維持英文姓名與屬性名稱；地圖名字牌會在切換語言時同步更新，無武將單位也會以本地化兵種名顯示（如投石車、衝車、工兵、糧車）。Worker、補給車、攻城器及沒有武將的單位不顯示肖像或屬性。
- 點選非目前 `Acting Side` 的單位時仍會顯示 unit info menu，但 action buttons 會隱藏，並在資訊中標示 `Command: Not Acting Side`，避免看起來像 command menu 無法彈出。
- Worker 被任何傷害來源消滅時，中央會顯示本地化的「勢力＋工兵隊被消滅」提示；可受攻擊的木橋因一般攻擊或火焰而耐久歸零、回復為 River 時，也會顯示包含座標的本地化木橋摧毀提示。兩者仍會同步寫入 Battle Log。
...
- top bar 的 Team A / Team B 總兵力會隨部隊損兵更新；一般攻擊、合擊、牆頂投放攻擊與 fire damage 造成的 `TroopCount` loss 都會扣到對應陣營。
  ...
  - 任一方 `Generals` 變成 `0` 時 battle 立即結束；畫面中央會顯示本地化的戰鬥結果覆蓋層（戰鬥結束、勝方、敗方已無武將率領戰隊的原因），並停止 End turn / command 操作。雙方均無武將戰隊的平手、以及攻方固守所有防禦據點的勝利原因也使用相同 locale keys；勝方與敗方名稱會用既有的陣營名稱格式化，避免顯示內部 `Team A / Attacker`。
...
- 右下角新增 prototype `Battle Log` 面板：
  ...
  - 每條紀錄的回合標籤以本地化的「攻方／守方」（英文為 `Attacker`／`Defender`）取代內部 `A`／`B` 代號，避免玩家把實作側別誤認為勢力或單位名稱。
  - 面板只顯示最新 80 條以維持可讀性，但「複製全部」會將本場自開戰至目前的完整 Battle Log 依時間正序放入系統剪貼簿，可直接貼到偵錯視窗、聊天室或文字檔；未 quick save 的紀錄仍會在關閉遊戲後消失。事件類別均使用 locale；武將名與兵種名也沿用戰場既有本地化資料（繁中如「夏侯淵／步兵」）。城戰 AI 的城門控制、破門、雲梯與出城反擊，以及移動、移動後攻擊原因、火計決策／執行、回合與 AI 控制／開局計畫事件皆使用繁中／英文 locale key；其他較少見的 prototype 事件敘述保留後續逐批整理。
...
- Gameplay 大地圖的浮動「軍報記錄」複製按鈕會依目前篩選寫入系統剪貼簿：選 `本勢力` 時只複製本勢力相關日誌，選 `全部` 時複製完整時間順序日誌；按鈕會分別顯示「複製本勢力」或「複製全部」，完成後顯示「已複製」。
...
- `SiegeAssault` 與 `MoatSiegeBattle` 會各部署一名攻方與守方 `Worker`：使用 `worker_idle_ne.png`／`worker_idle_se.png` 的四格 idle animation，具備低近戰與移動能力。
  ...
  - `Bridge` 可在 MoatSiegeBattle 的護城河上建橋，或修復受損橋面。半 HP 的施工中橋面會另外保存 `IsBridgeUnderConstruction`，攻守雙方均不可通行，必須由 Worker 修到滿血才完成。
- Worker 可在 `MoatSiegeBattle` 的相鄰 `Moat` 格建造橋、修復受損橋面、修復受損城門及移除 `Trap`；橋梁需兩次 `Work` 才會完成，第一次為半 HP 的施工中橋面且不可通行，第二次回滿 HP 後才可通行。
  ...
  - 已完成的石橋／木橋受損後，只要 HP 仍高於 `0`，一般 battle team 與 Worker 仍可通行；HP 不高於最大值 `50%` 時，每個嚴重受損橋格消耗 `3` 行動力及 `3` 格移動格數，可達範圍、A*、移動預覽、實際扣除與 AI 路徑均使用相同成本，且所有攻城器與糧車不可通過。石橋 HP 高於 `50%` 時攻城器與糧車可通行；木橋無論 HP 多高都只供一般 battle team 與 Worker 通行。HP 歸零後攻守雙方皆不可通行。
  - Infantry、Spearman、Guard 可對四向相鄰、已完成但受損的橋使用 `Repair Bridge`：每次恢復 `150 HP`、消耗 `5` 行動力並視為本回合已攻擊；不限制附近是否有敵軍。這些兵種不能續建施工中橋面，也不能在橋毀後新建或重建；Worker 仍每次恢復 `450 HP`，並獨佔建橋／重建能力。
  - AI 的 Infantry、Spearman、Guard 使用與玩家相同的 `Work` action intent 與合法性規則；相鄰受損橋會依低耐久危急度、修復後是否跨越 `50%` 重損門檻、附近友軍數量及武將 Intelligence 計分，再與攻擊、移動、Worker 工程及據點候選共同排序，不設固定修橋優先。
  ...
  - 橋梁有獨立 HP，所有戰鬥單位可依其結構傷害攻擊橋面；HP 歸零時橋面會移除，攻城護城河橋還原為阻擋移動的 `Moat`，野戰木橋則還原為 `River`。施工、修復、破壞完成後立即更新 `MoatLayer`、`ObjectLayer` 與通行規則。
...
- `MoveRange` 同時是每個單位在其所屬行動方內累積可走的最大格數，不是每次點選 Move 的獨立上限。一般移動每跨一格扣除 `1` 點剩餘移動格數，嚴重受損橋格則依橋梁規則扣除 `3` 點；因此步兵的 `4/4` 在一般地先走兩格後會成為 `2/4`，即使行動力仍足夠也不能在同一行動方再走超過兩格。普通移動本身不會完成該隊行動：只要仍有 Energy 與剩餘移動格數，仍可再次 Move，或改用攻擊、Guard、計略等合法指令；移動動畫完成後，玩家的指令選單會自動重新顯示，即使單位位於關閉城門的遮蔽區。攻擊、Guard、Hide、補給、工程與城門切換等終結行動才會標記已行動。騎兵維持自己的較高 `MoveRange`（目前為 `6`），同樣受行動力與累積移動格數雙重限制。battle quick save/load 亦保存剩餘移動格數。
...
- 補給車的恢復／維修與補充武器均消耗 `5` 行動力並完成行動；玩家與 AI 的補充武器共用相同 Action Intent／Executor 路徑。AI 會先嘗試原地補給，再將可用的一般攻擊、合擊、一般補給、補彈與可達防禦堡壘放入同一批候選行動，以預計傷害、擊破獎勵、目標武將價值、補給效益、合擊友軍行動力機會成本及堡壘目標分數計分；同分或相近分數以可重現的小幅變異打破固定行為。堡壘不是固定優先：攻方奪取未佔堡壘、守方回奪失守堡壘／防衛受威脅堡壘的分數均扣除距離，並受該 battle team officer Intelligence 影響；最後一座攻方未佔領堡壘另有勝利加分。失守／敵方堡壘上的敵軍也會獲得額外 attack score。若可用行動力扣除路徑成本後仍保留 `5`，則會先完成移動動畫，再直接攻擊規劃時保留的目標（不在移動後重選）；Battle Log 會記錄該 score／variance，若攻擊因單位狀態、目標或射程失效而取消，也會記錄明確原因。若選擇純移動，log 會額外列出路徑耗能、移動後剩餘行動力／移動格數，以及沒有可用移動後攻擊方案的原因。否則僅移動至最接近目標的最遠合法位置並完成行動。
...
- 投石車一般攻擊的動畫順序為：投石車攻擊 → 石彈飛行完成 → 目標受傷與傷害數字 popup；弓兵與弩兵同樣在箭矢命中後才觸發傷害結算、戰鬥紀錄與 Guard 判定。弓／弩射向相鄰一格時維持直射；相隔至少兩格時，箭矢以隨距離增加、設有上限的二次 Bézier 拋物線飛行，並沿飛行切線轉向，讓遠射有自然拋射觀感而不改變命中結算時機。
- 頂欄會依目前語系顯示戰場名稱、日期與 Field Battle AI 控制鈕；繁中日期格式為 `YYYY 年 MM 月 DD 日`，英文維持 `YYYY Apr D`。
- 戰鬥開始會先聚焦攻方發言武將並播放開戰台詞，再聚焦守方發言武將並播放開戰台詞；兩段發言結束後才在畫面中央顯示目前勢力的本地化回合橫幅（如「曹操軍回合／董卓軍回合」）。其後每次行動方切換仍立即顯示橫幅。橫幅於 `1.5` 秒後自動關閉，並在顯示期間鎖定戰場與 UI 輸入。
...
- 第 `10` 日 Night 結束後仍未達成勝利、撤退或投降條件時，戰鬥只會暫停，不強制判定勝負或平手。系統保存兵力、傷兵、HP、士氣、Energy、彈藥、Gold／Food、火勢、可變地圖／建築耐久、據點歸屬、環境 seed／step、待命援軍及 Battle Log，先顯示「本月戰鬥暫停／下月續戰」結果，再返回 Gameplay 大地圖；Debug 的一日上限測試走同一條 snapshot／續戰流程。
- 下一個戰略月重新載入既有 snapshot 時，保留部隊位置、城門／橋梁／火勢及其他地圖狀態，但日期一律改為新月份的本月戰日 `1 / 10`、時段回到 Dawn、行動方回到攻方，並清除上月的已行動／已使用指令狀態與重置雙方 Energy／移動力。跨月傷兵恢復與為恢復支付的 Food 以 Campaign 資料為準回寫到戰場戰隊，不能被舊 snapshot 覆蓋。
- 大地圖採用「本月指令階段 → 本月戰鬥階段 → 才進下月」。未結束戰役的目標城市會直接疊加 `assets/map/battle_icon.png` 戰鬥圖示；不使用紅色環或 `BATTLE` 文字。月末戰場暫停後會回到大地圖並進入本月戰鬥結算階段：城市一般指令全部停用，只能檢視與結束回合。按結束回合後才會載入同月下一場待處理戰鬥；若沒有剩餘戰鬥才進入下月、解除指令鎖定。援軍與其他城市指令只在下一月的指令階段派遣。玩家與 AI 可先下本月指令，下一次按結束回合時才載入本月可續戰的 snapshot。攻城準備階段仍停留大地圖等待玩家決定。世界目前只有 Year／Month，未加入世界日曆前，UI 應顯示「本月作戰第 N／10 日」，不要將其誤寫成完整世界日期。
- 戰役資料開始以 `TroopType.Engineer` 表示原有的攻城兵人員；舊 `siegeTroops` 存檔欄位會映射為工兵。`BattleEquipmentType` 用於無武將統率的戰役裝備（補給車、衝車、雲梯、投石車），並有獨立的工兵支援部署資料。每方補給車上限為 `1`；器械在完成 UI／戰場遷移前仍沿用既有三項器械配置。
- 登用人士不是即時結果：玩家必須自本城指派一名可用武將為登用使者；確認後使者會鎖定至月底，登用接受判定與資源支付均在本月結束的待辦命令結算。每城每月只能安排一項登用。
- 城市 `View` 的武將名冊會使用戰役的有效位置：來源城市不再列出已投入未結束戰役的武將；點選帶有戰鬥圖示的目標城市時，則列出該戰役攻守雙方所有具武將的參戰／預備／行軍戰隊。這只改變檢視位置，不覆寫 officer 的持久 `CityId`，故戰後結算、撤退及傷兵回歸仍以既有資料處理。
- 戰鬥城市的參戰武將名冊會另顯示「勢力」欄，直接採用戰役戰隊的 `FactionId`，因此能分辨本國、同盟與敵方同盟援軍；敵方參戰武將仍只公開姓名、官職、戰隊兵種與「攻方／守方－位置」等可觀察戰場資訊，年齡、忠誠及能力數值仍按一般情報規則顯示 `??`，除非已有完整情報或開啟 God Mode。
- 戰役中的武將、戰隊、軍資及來源城市兵力必須被鎖定，不能同時再用於其他命令；所有一般城市指令的武將候選清單（包括下一月重新開放的 Attack、Move、內政與人事）都會排除未結束戰役的參戰、預備、行軍與邀請使者，避免 UI 顯示可選、正式結算卻拒絕的情況。戰場目標城市在戰役結束前亦不能被一般即時攻擊結算重複處理。
...
- 玩家守城時，防禦配兵 UI 在「確認守城」之前提供相鄰城市的援軍入口。本國相鄰城切換至該城的武將／兵種／兵力編輯後加入請求；同盟相鄰城必須選擇一名未編入守軍的本城使者，再選擇請求規模。已選擇或已儲存於同盟援軍請求的使者會立即從守軍武將清單及守軍配兵移除，不能同時擔任守軍與使者。使者不另行花費外交移動日，但在請求被接受至戰役結束期間鎖定，並以其 Intelligence／Charm 加入同盟君主的即時接受判定；接受後盟友 AI 會在總請求規模內，自動決定借給使者統率的步兵數與由盟軍武將統率的其餘援軍。使者隊為玩家控制、步兵仍屬盟友來源城；盟軍武將隊維持盟軍 AI 控制。兩類隊伍都隨戰役建立進入 `Traveling`，直接鄰城從戰役第 `1` 日起計算 `2` 日 ETA，而不是等待月末；戰後步兵返還盟城，使者返回己方守城城市。
- 每筆同盟請求於確認守城後，會先寫入 Gameplay Log，並以「同盟援軍回覆」視窗顯示同意或拒絕；回覆視窗沿用既有戰報／勢力結果的深色面板、金棕框線與確認按鈕樣式，而非 Godot 原生灰色提示框。所有回覆確認後才切換至戰場。盟軍正式援軍依請求規模選擇 `1`／`2`／`3` 名 Leadership＋Combat 較高、再以 Intelligence 排序的盟軍武將，避免把所有可用武將切成大量零碎小隊；使者借調步兵與這些 AI 部隊合計不超過請求上限。
- 援軍來源為同盟城時，按鈕標示為「提出請求」；來源為己方鄰城時標示為「編組援軍」，進入配兵後標示為「儲存援軍」，避免把外交請求誤解為直接出兵。
- 來源城市必須與戰場有完整友軍路線、沒有被包圍或參與另一場戰役，且派出後仍保留依周邊威脅計算的最低守軍。每座城市每個戰略月最多派出一次援軍。
- 下一月指令階段若同一攻方勢力的城市對既有持續戰役目標使用 Attack，UI 會改顯示「增援／確認增援」；確認當下直接建立 `ReinforcementOrder`，而非新增第二場攻擊。命令確認時立即從來源城市扣除並鎖定武將、兵力與軍資，建立 `ReinforcementOrder`；尚在行軍中的資源不能再被其他命令使用，也不會提前加入戰場補給。
- 援軍來源城市預設必須保留全域平衡常數 `MinimumCityGarrison = 1,000` 的守軍，防止城防被完全抽空；這是目前程式常數而非城市 UI 設定。若增援失敗，UI 必須顯示具體原因：本月已派出、沒有友軍路線、派出後守軍／最低守軍、配置無效、兵種／金糧不足或攻城器不足。
...
- 援軍只在己方時段開始前的 Dawn 進場。野戰援軍一律在己方兩條外側地圖邊緣的可通行、未佔用格進場（攻方 NW、守方 SE）：優先使用角落入口；若該格為山、水或障礙，場景會沿同側邊緣帶尋找下一個可用格，不會回退至原始攻守部署區或直接生成在既有友軍旁；整個邊緣帶均無可用格才維持 Reserve。City Battle 則依既有外側入口規則，守方援軍不能直接生成於內城。只有原本的城市守軍可在內城開始。入口受阻時進入等待隊列，不可重疊生成或直接出現在敵軍旁邊；中央事件提示與戰鬥紀錄會列出抵達援軍的勢力、武將／兵種／兵力及 Gold／Food。援軍實際部署後，戰鬥紀錄另以「原有 + 援軍 = 最終」列出該側 Gold／Food，且 top bar 立即同步最終值。若當前有攻守方回合 banner，援軍中央提示會等 banner 隱藏後才顯示，避免重疊；援軍中央提示關閉後，才建立各在場武將的士氣 `+/-` popup，並有 `65%` 機率由本批實際入場、具武將的援軍隨機一人說本地化到達台詞，確保效果可見且不為 Reserve／無武將隊伍觸發；同一 Dawn 的多筆援軍會依勢力合併為一則提示，避免重複標題堆疊。
- 援軍實際部署到戰場時，該側所有在場本國／盟軍戰隊獲得 `+5` Morale，對側所有在場戰隊承受 `-4` Morale；每筆援軍只結算一次，僅處於 Reserve 而尚未進場時不觸發。此效果表示士氣振奮與敵軍壓力，不改變兵力、Food 或原始 ETA。
- 戰役提前結束時，未抵達援軍與軍資停止行軍並返回來源城市；已抵達但未使用的本國軍資於戰後按正式結算規則返回控制方城市。

### 11.3 戰前外交、盟國與其他國家援助

- 玩家在指令階段發起的「締結同盟」會於月底結算；無論成功、遭拒或因目標失效而未成立，均會寫入「本勢力」Gameplay Log，並以既有戰報風格的「結盟結果」確認視窗顯示實際結果，避免玩家只從日誌篩選或外交關係表推斷。
...
- 第一版守城 UI 的正式同盟援軍請求限於相鄰、有效 `Alliance` 的來源城，沒有額外等待回合；接受／拒絕與 AI 配兵在確認守城後同步完成，並記錄在戰役 invitation 資料。使者、非相鄰國家與資源型請求仍保留給後續完整戰前外交流程。
...
- 超過上限的已抵達援軍進入 Reserve Queue，不算作可直接操作的在場單位；在場部隊潰退、撤退或被消滅後，空出的名額才可由預備隊於下一個合法 Dawn／己方階段開始前補入。援軍由己方初始部署區中可用的外側格進場；`BattleScenarioDefinition` 可指定較優先的入口格，但該格仍必須屬於同方部署區，否則會略過。入口沒有可用格時，該援軍維持 Reserve，不會生成到敵軍旁或被當作陣亡，Battle Log 會明確記錄「入口受阻」。
...
- 守方野戰勝利會擊退攻方並結束本次城市攻擊；守方野戰失敗後，每支存活戰隊可依合法路線選擇退入目標城市、撤往友方鄰城、繼續作戰或投降。玩家戰隊抵達守方撤退區後，目的地視窗會把「退回城內防守：目標城」列為首項，再列相鄰己方／無主城市；選此項會保留同一 Campaign 的兵力、傷兵與武將為 `InnerCity` 戰隊，供攻方野戰勝利後的後續城戰使用。只有實際抵達守方撤退區／內城入口且未被包圍的戰隊可進入後續守城戰，不能在野戰敗北時自動傳送回城。
...
- City Battle 進行期間，不論本國或外國、攻方或守方，後續抵達的戰隊均先到達城市外圍合法 entry zone，不可直接傳送到城內、城牆、城門格或敵軍旁邊。守方援軍的預設入口為城牆外側的地圖邊緣；依城牆朝向選擇 `x = 0`、`x = 24` 或其中實際位於城外的一側，不在城門外一、兩格直接出現。
- 守方由玩家控制的使者借調步兵與本國援軍，頭頂陣營三角標記沿用原始守軍藍色；守方盟軍 AI 援軍使用青綠色；攻方援軍使用黃橘色。此為援軍辨識用途，不改變攻守陣營、控制權、路徑或攻擊判定。
...
4. **已完成—snapshot 與場景切換主幹**：第十個完整作戰日後將戰場 DTO 寫入 Campaign、返回 Gameplay，下一月可由「持續戰役」重載；Field → City 使用同一 Campaign 並清除不相容地圖 snapshot。
   - 戰役真正結束後會在 Gameplay 載入獨立 `BattleReportDialog.tscn` 戰報，逐一列出攻守兩方的出征、陣亡、傷兵、撤回／返還與戰場殘存兵力，以及勝方與俘虜；攻、守統計卡標題分別顯示該側現任君主，例如「攻方：張角」與「守方：橋瑁」。並顯示各方的金、糧戰時消耗與獲得。攻方攻陷城市時，「獲得」只計守方戰後仍留於戰場資源池、且隨奪城轉移的金糧；己方未用完的補給不被誤列為戰利品。撤退而先回城的生還兵力會保留在「撤回／返還」，不會再被誤顯示為零兵。戰報沿用主地圖的深色面板、金棕框線與指令按鈕樣式，並以醒目的「戰果：勝利／敗北」直接顯示玩家結果（金色勝利、紅色敗北），不要求玩家由勝方勢力名稱自行判斷。戰報首次開啟時，玩家勝利播放 `bgm_victory_01.ogg`、敗北播放 `bgm_defeat_01.ogg`，且遵從 BGM 開關與音量；關閉戰報時先停止勝敗曲，再播放 Gameplay 的 `bgm_world_map_01.ogg`／`bgm_world_map_02.ogg` 曲目。玩家確認戰報後才進行該戰役產生的俘虜處置。AI 攻擊玩家城市時，防守確認會保留原 Attack queue 項目並立即以玩家配置建立／載入戰役，不能因外交提案或月結算而遺失。戰日用盡而跨月續戰只顯示戰況，不能處置俘虜。
   - 同月若還有已排定的攻擊或玩家防守，返回 Gameplay 時維持「本月戰鬥結算中」；一般城市指令維持鎖定，玩家按結束回合才依序載入下一場，所有同月戰鬥處理完才進入下月。
   - Option UI 提供持久化的「AI 決策偵錯」開關，預設關閉。開啟時向 Gameplay 日誌追加每座 AI 城的評估起始狀態、最終執行結果與實際決策理由；外交理由列出類型、目標勢力與命中的門檻／兵力／關係值（停戰、毀約、索貢、同盟或送禮），攻擊理由列出目標、派遣兵力、情報是否可見、君主野心與實際門檻。已知情報時，進攻門檻為 `clamp(550 - 野心 × 5, 50, 450)`，野心 50 維持既有 +300；無情報時，盲攻門檻為 `clamp(3000 + (50 - 野心) × 8, 2600, 3400)`。關閉時所有 AI 理由日誌均不顯示，不因目標是否為玩家而例外。開啟時，目標為玩家的攻擊與外交偵錯行會標記為「本勢力」相關，故可在日誌的「本勢力」篩選看見；其他 AI 偵錯行則在「全部」篩選查看。不修改 AI 評分、隨機性或行動結果。所有 Gameplay 日誌均由共用入口加上當前年月，例如「184年2月｜…」，避免月結、災害、移動、偵查與 AI 偵錯各自使用不一致的日期格式。
   - 所有已結算的間諜任務（成功、失敗與身份暴露）均會列出發動勢力君主與執行武將，例如「184年2月｜【張角】管亥 偵查『濮陽』成功」或「184年2月｜【張角】何儀 對『長安』執行『偵查敵情』失敗，且身份暴露」；不再誤列被偵查城市的君主或行政官，且日期不會重複顯示。
   - 攻擊與守城配兵的每位武將列均提供 locale 化的「最大／Max」按鈕。按下後只填入該兵種扣除其他已配武將後的剩餘兵力；切換兵種或修改數量會立即更新其他列的可填上限，避免同一兵種重複配置。兵種選單保留彈性寬度，但以固定最小寬度搭配緊湊的最大按鈕，減少畫面浪費；最大按鈕沿用確認指令按鈕主題。攻擊目標選單以「【君主】城市」顯示，守城時亦會標示來襲君主，避免只看城市名稱而無法辨認敵軍歸屬。戰場頂欄的行動方、攻方與守方也以「【君主】」顯示，不再附加「軍」字；戰鬥紀錄的「本勢力」篩選固定依玩家實際攻守方過濾，不能隨目前行動方錯誤切到敵軍紀錄。
   - 同一結算月份內，同勢力對同一目標城市的多筆出征，在互動戰役建立前合併為一場聯軍戰役。每支部隊保留原本武將、兵種與來源城市，因此戰後撤退與回歸仍會回到各自來源；資源會合併為攻方戰役資源。合併命令從待結算佇列移除，避免戰後重複建立第二場戰役。戰役開始後才追加的出征仍走既有援軍流程。
   - 戰役撤退目的地會列出可返回的來源城與戰役錨點相鄰的己方／無主城市；守方處於 Field Battle 且已抵達撤離區時，目標城額外以「退回城內防守」列為首項，選取後戰隊留在同一 Campaign 的內城等待後續城戰。City Battle 中目標城正是交戰地點，守方不可把它當作「返回來源城」再選一次；其他撤退仍只可選相鄰合法城市。若沒有合法目的地，撤退不會執行。
   - 在戰鬥結算中查看戰役城市的武將清單，參戰者的「狀態」會顯示公開戰況 `攻方／守方－在場／城內／預備／行軍／撤往鄰城／已潰滅／被俘`；仍不公開敵方兵力、能力或其他受情報遮蔽數值。
...
11. **已完成—戰後俘虜、繼承與復國**：Campaign 奪城俘虜會與戰報回到 Gameplay；玩家勝方以既有俘虜面板選擇招募、釋放、監禁或處決，AI 勝方則依能力與忠誠自動選擇四種處置。攻城時同一守將可能同時存在於城市名冊與戰役戰隊，結算以 officer ID 集合確保只俘虜、排入處置及列入戰報一次；戰報顯示亦會再去重以兼容舊存檔。AI 處置結果（招募、釋放、監禁、處決）不再被丟棄：即時戰略戰會在攻防結果後、Campaign 則在戰報顯示前，各自新增 Gameplay Log；每一行都以處置方的現任君主標記，例如「【丁原】已釋放武將『何進』」，故俘虜由「下獄」轉為「在野」時可追溯釋放事件。若被俘者為君主，俘虜面板會明示其君主身分與原勢力已進入繼承程序；戰鬥結果日誌也會列出「君主被俘」及 AI 繼位／勢力瓦解結果。即時戰略攻擊會在戰鬥判定前、Campaign 則在建立時，凍結攻方原君主的 officer ID；故被俘觸發自動繼位後，日誌與君主更替視窗都會正確記錄發動出征的原君主，而非繼位後的新君主，並可隨 locale 顯示姓名。AI 勢力的君主被俘而自動產生繼位者時，無論是即時戰略結算或 Campaign 戰報確認後，都會另外排入既有戰報風格的「君主更替」確認視窗，不會只藏在 Battle Log 文字內；視窗會先列出年月、原出征君主、來源城、目標城及被俘君主，明示觸發繼位的戰役原因，再列出前任與繼任者。本月攻擊結算會在結果視窗排隊期間暫停；玩家確認君主更替／勢力滅亡後，才繼續顯示下一場守城配兵或處理下一筆命令，故兩個 modal 不會重疊。玩家君主被俘或死亡時，繼承視窗會明示前任姓名與原因，要求玩家從全勢力所有存活、未被俘且未參戰的武將選擇新君主；其候選表會顯示統率、武力、智力、政治、魅力、忠誠，以及與前任君主的關係類型；關係資料即使只在任一方記錄亦會辨識，現有 `family` 類型顯示為「親族」。AI 的死亡／被俘自動繼位使用相同全勢力資格：有合格親族時必先在親族中按能力與忠誠選擇，無親族才由非親族選擇；前任資料遺失時才只比較能力。確認後的「君主更替」結果視窗亦會分別標示前任被俘或死亡。人事指令與 AI 主動改立亦會標示為人事任命／勢力人事決策：前者僅能從本勢力未被俘、未死亡、未參戰的現役武將選擇，確認後解除原君主的 Lord 職務並設置新君主；同樣會寫入日誌與顯示君主更替提示。AI 每月在安定狀態評估改立：不處理進行中戰役或外交命令的勢力，高齡或接近已知壽命的君主，僅在合格候選人的綜合能力明顯較高（或君主忠誠過低）時才讓位；候選人要求忠誠至少 70，計入統率、智力、政治、魅力、野心及家族關係。改立後原君主保留為一般武將、忠誠 +5；新君主野心超過 70 時，其餘部將會承受小幅忠誠下降，且六個月內不再改立。AI 改立會寫入 Gameplay Log 並顯示既有君主更替提示。勢力只保留一位君主，名稱、外交與頂欄立即更新。釋放時優先回到被俘前的 `HomeCityId`，以在野身分停留。因無繼承人而亡國的被俘君主會保留原勢力 ID；後續在野移動進入無主城市時會立即佔城、恢復君主與原勢力。無繼承人時勢力城市轉無主。玩家除失去所有城市外，若仍有城市但所屬武將全數死亡，也會敗北。

12. **已完成—勢力結果提示**：Gameplay 偵測到原本持有城市的勢力失去全部城市時，會在既有日誌之外以同戰報風格的 modal 顯示勢力滅亡；日誌、外交關係表與勢力顯示一律解析現任君主，不使用舊式 `xx軍` 後綴。多個勢力會依序顯示。玩家因失去全部城市或沒有存活的所屬武將而敗北時，顯示「遊戲敗北」modal；確認後隱藏地圖與 Gameplay HUD、恢復 Main Menu 與主選單音樂。若戰報仍在畫面上，結果提示會等戰報關閉後才顯示。

13. **已完成—共用存檔欄位的 Campaign 戰役續戰**：獨立測試戰場不支援 Save／Load。Campaign battle 的選項面板以一個「儲存／讀取」入口開啟既有 `SaveLoadDialog.tscn`，使用 Gameplay 相同的 `slot01.json` 至 `slot10.json`；視窗內保留選 slot、描述、儲存與讀取按鈕。視窗初次開啟會置中，標題列可拖曳，並限制於目前 viewport；dialog 會攔截滑鼠輸入，不能在其上操作地圖。儲存前將目前 `BattleSaveData` 寫入 active campaign 的 `BattleSnapshotJson`，再存完整 `WorldState`。從任何位置讀取一般 Gameplay slot 時維持 Gameplay；若 slot 內有玩家參與、未結束且含 snapshot 的 Campaign，會自動重新開啟對應戰場、套用 snapshot。戰鬥結束後仍按既有流程回 Gameplay 處理戰報與後續回合。所有 Save／Load slot list 與儲存詳情會標示「大地圖」或「戰役戰鬥」；類型由未結束 Campaign 的 snapshot 判定，不依賴玩家輸入的描述。

14. **已完成—玩家戰役撤退目的地對話框**：玩家帶武將戰隊抵達撤離區後，會以 `RetreatDestinationDialog.tscn` 顯示有效的來源己方城市及相鄰己方／無主城市；Field Battle 守方另會把目標城列為「退回城內防守：城市」，選擇後不會把部隊結算回大地圖，而是保留為同一 Campaign 的內城守軍。來源城市永遠優先列出為「返回來源城：城市」，每個其餘候選都是明確的「撤往：城市」按鈕，而非不可辨識的文字。使者借用盟軍步兵時亦開啟此對話框，但候選依使者的原勢力判定；選定城市只接回使者，盟軍步兵另行返還盟軍來源城。此候選集合同時由 `BattleCampaignService` 驗證與執行，AI 也使用相同規則。對話框沿用主遊戲深色面板、金棕標題列與指令按鈕樣式，初次置中、可拖曳並攔截戰場輸入。沒有合法目的地時不開啟空白選單，直接顯示「沒有可前往的城市，無法撤退。」

15. **已完成—回合 banner 行動鎖定**：開戰時由第一句開場武將台詞（包含台詞前的短暫鏡頭延遲）起，到 `TurnBanner` 完全隱藏為止，同步啟用全畫面透明輸入遮罩與戰場 `_Input`／`_UnhandledInput` 狀態門檻；玩家不能點選地圖、部隊、指令、Options 或拖曳面板。相同狀態亦會封鎖 AI 的單步決策、AI 控制切換及結束回合，並停用相應按鈕，因此玩家與 AI 都必須等 banner 完全隱藏後才可控制／執行部隊。其後每次攻守回合切換亦使用同一門檻。banner 由同一 serial 計時器隱藏時才解除鎖定並刷新指令，避免連續回合的舊 timer 提早恢復輸入或行動。

16. **已完成—繼承血緣優先**：`relationship_type` 的 `family,blood` 與 `family,non-blood` 分別表示血親與非血親。玩家繼承表會分別顯示「血親」與「非血親」及依本局年份計算的年齡；AI 在君主死亡或被俘時，從全勢力合格武將依血親、非血親、無關係排序，再於同一關係層級比較能力與忠誠。關係資料只要任一方向有記錄即可辨識。開發期間，任何存檔載入都會從目前 `data/person/officer_data.json` 重新同步每位武將的 `relationship_type`；城市、所屬、忠誠、經驗與其他存檔中的戰役狀態仍保持原值。

17. **已完成—戰役俘虜批次繼承**：即時戰略攻城與 Campaign 戰役均先處理該戰役全部戰死與俘虜，再為每個受影響勢力只執行一次繼承。已於本戰死亡的武將會從俘虜候選排除，故不會再同時出現「戰死、被俘、被釋放」；同城守將亦不會被先選為新君主、隨即在同一戰役再次被俘。若全數合格人選均已死亡或被俘，才按既有規則進入勢力滅亡或玩家繼承流程。

18. **已完成—武將選擇表排序與所在地篩選**：共用武將選擇表支援點擊任一欄位標題排序；文字欄首次點擊為升冪、數值欄首次點擊為降冪，再點擊同欄位切換方向，且數值欄依數值而非字串排序。候選武將分布於多個所在地時，表格顯示「所在地」下拉篩選。登用人士額外顯示勢力（現任君主名稱；無所屬勢力則為在野）、城市、狀態、年齡、忠誠、野心、統率、武力、智力、政治、魅力與戰鬥等資料；`CityId = 0`、未落在地圖城市的武將不會列為登用候選。在野武將的忠誠顯示 `--`，並於忠誠排序時作為無數值處理。解除武將候選表保留官職、狀態，並顯示年齡、忠誠、統率、武力、智力、政治、魅力與戰鬥，讓玩家在執行前完整比較。軍事面板確認移動或攻擊時，若沒有相連的合法友方或敵方城市，會直接在面板內顯示原因，而不只寫入 Gameplay Log。

19. **已完成—攻擊面板軍師建言**：攻擊面板將「軍師建言」按鈕置於底部、與「確認攻擊」並列。按下後優先由本勢力仍可用的首席軍師發言；未任命或該軍師不可用時，改由來源城市智力最高的本地武將建言。建言會開啟既有參謀對話框，以武將肖像、姓名、職位與訊息方式呈現。建言只讀取目前目標與玩家已配置的出征武將、兵力、糧草、外交關係及已取得的城市情報，不會自動勾選武將、改變配兵或送出命令；目標城不在已知情報內時不顯示守軍與城防數字，而是建議先偵查。已知情報時則以出征兵力、守軍及城防提示「不宜強攻／謹慎出擊／可考慮出擊」；未配兵、未帶糧或將撕毀仍有效的同盟／停戰時，各自優先提示準備、補給或外交風險。

20. **已完成—外交目標關係值**：外交面板的「目標勢力」下拉清單在每個勢力名稱後顯示目前關係值（正值附 `+`）與關係狀態，例如「劉焉｜關係 +35｜同盟」，使玩家可在提出同盟、停戰或其他外交行動前比較目標。

21. **已完成—使者與賞賜人選能力比較**：外交使者選擇表顯示城市、狀態、忠誠、魅力、智力及政治；年齡與統率不屬外交選派的判斷資料，故不顯示。賞賜武將選擇表顯示職務、任命、城市、狀態、年齡、忠誠與全部能力。兩者均沿用共用表格的欄位標題排序，故可按忠誠或所需能力比較後再選人。

22. **已完成—內政計畫丞相建言**：內政面板將「丞相建言」置於「新增計畫」旁。按下後優先由本勢力仍可用的丞相發言；未任命或該丞相不可用時，改由來源城市政治最高、再以智力排序的本地武將建言。建言使用既有參謀對話框的肖像、姓名、職位與訊息呈現方式，讀取目前選定的內政項目、每月投入金、執行月數及城市農業、商業、城防、防災、Gold／Food 狀態，提示投資優先順序；不會自動改變項目、武將、金額或新增計畫。

23. **已完成—徵兵面板招募建言**：徵兵面板將「招募建言」置於確認按鈕旁。按下後優先由本勢力仍可用的首席軍師發言；未任命或該軍師不可用時，改由來源城市智力最高、再以統率排序的本地武將建言。建言使用既有參謀對話框的肖像、姓名、職位與訊息呈現方式，讀取目前選定的兵種、徵兵數量、可徵上限、Gold／Food 成本與兵種特性；接近可徵上限時提醒保留人口與資源，騎兵、攻城兵、遠程兵及一般兵種各有不同的招募取捨提示，不會自動改變兵種、數量或確認徵兵。

24. **已完成—外交計畫建言**：外交面板將「外交建言」置於確認按鈕旁。按下後優先由本勢力仍可用的首席軍師發言；未任命或該軍師不可用時，改由來源城市魅力最高、再以智力排序的本地武將建言。建言使用既有參謀對話框的肖像、姓名、職位與訊息呈現方式，讀取目前選定的外交行動、目標勢力、關係值、條約月數、Gold／Food／Horse 條件、使者能力、對方君主、來源城市民心與雙方兵力。贈禮明示月底結算與關係提升 `3..25` 的預期，毀約明示立即回到中立與關係 `-18`；同盟、停戰與索取則顯示以現有資料估算的成功機率，以及有利／謹慎／風險偏高的策略提示。建言不會自動變更或送出外交命令。

25. **已完成—間諜計畫建言**：間諜面板將「間諜建言」置於「確認間諜」旁。按下後優先由本勢力仍可用的首席軍師發言；未任命或該軍師不可用時，改由來源城市智力最高、再以魅力排序的本地武將建言。建言使用既有參謀對話框的肖像、姓名、職位與訊息呈現方式，讀取目前選定的間諜行動、目標城市、執行武將、敵城武將平均智力／魅力、敵城城防、來源城市民心與城防、執行者間諜階級，顯示以目前資料估算的成功率與身分暴露率；數值仍可能受裝備等實際結算資料影響。偵查明示約三個月的城內情報，破壞明示 Gold／Food／城防的實際隨機損失範圍，煽動明示民心損失範圍，暗殺則明示指定目標與較高暴露風險。建言不會自動選擇武將、改變行動或送出間諜命令。

26. **已完成—賞賜建言**：賞賜面板將「賞賜建言」置於「確認賞賜」旁。按下後優先由本勢力仍可用的丞相發言；未任命或該丞相不可用時，改由來源城市政治最高、再以智力排序的本地武將建言。建言比較本城可賞賜的非君主武將，優先推薦忠誠較低、野心較高者；選有道具時，另將武器、馬匹、書籍、寶物分別偏向武力／戰鬥、統率／戰鬥、智力／政治、魅力／政治較適合的人選。建言會明示目前選定武將、Gold／Food／道具帶來的忠誠增量、忠誠上限 100、道具會直接裝備給受賞者，以及建議人選是否與目前選擇一致；不會自動改選武將、改變資源或送出賞賜。

27. **已完成—人事指令建言**：指派職位、改立君主、太守授權、解除武將與索回道具的確認按鈕旁均提供對應建言。優先由丞相、否則由本城政治最高的可用武將以既有肖像式參謀對話框發言。任命建言顯示接任／替換效果與候選的政治、智力、統率；繼位建言顯示候選能力、忠誠與野心；太守建言區分未授權、玩家指定的半授權、太守自行規劃的完全授權；解除建言明示在野、職務清除與道具返還；索回建言明示移除的裝備能力與忠誠效果。建言不會自動送出任何人事命令。

28. **已完成—未選武將的任命建言**：指派職位面板即使尚未選擇武將，仍會依目前職位下拉選項，在本勢力可任命、非君主武將間評分並推薦最佳人選；太守依政治、智力、魅力，軍師依智力、政治、魅力，丞相依政治、智力、魅力，首席軍師依智力、統率、戰鬥評分。已選武將時，建言並列目前人選與最佳人選的分數，明示目前選擇是否已是最佳、或只是可任命但較弱的選擇；分數只供建言比較，不改變既有任命規則。

29. **已完成—改立君主建言與清除選擇**：改立君主面板的「繼位建言」與「清除選擇」均為 locale 化按鈕。清除選擇後，確認改立會在同一面板以警告提示先選擇武將；未選擇時建言依全部合法候選推薦綜合最佳人選，並列出統率、武力、智力、政治、魅力、戰鬥、野心、年齡與現任君主關係。血親、非血親關係與約 30 至 60 歲的成熟年齡各有推薦加分；71 歲以上或過於年輕者會降低建言評分。已選擇時建言顯示該候選的資料及綜合評分，與現任君主比較，並在有更佳合法候選時明示其姓名與分數；忠誠不納入此處的評分或顯示。這些評分僅作決策輔助，不改變原有繼位資格或自動繼承規則。

30. **已完成—半授權計畫建言**：太守授權面板選擇半授權時，授權建言會以與完全授權 AI 相同的城市需求評估，考慮農業、商業、城防、防災、人口、Gold、Food、兵力、太守能力與已進行的內政，推薦一個內政項目及 2 至 4 個月的執行期；同時比較目前玩家指定的項目／月數是否一致。建言只提供建議，不會修改半授權的下拉選項或月數。

31. **已完成—解除武將的可決策建言**：解除武將面板的「人事建言」改為結論、預期結果、風險、替代方案四段式。建言會讀取被選武將的既有任命、丞相／首席軍師職位、統率、智力、政治與持有道具數量；未持有職務時提示可解除，持有職務但本城沒有可用非君主候補時提示暫不建議解除，否則會明示候補武將並建議先補位。候補評分依太守、軍師、丞相、首席軍師的相應能力需求選擇，不採用忠誠作為解除判斷，也不會自動任命、解除或移動任何武將／道具。

32. **已完成—索回道具的分階段建言**：索回道具面板的「道具建言」會依選擇狀態提供不同內容：未選持有人時，若本城有持有人與道具則提示一組可先檢視的對象，否則明示沒有可索回目標；已選持有人但尚未選道具時，會從其持有道具中推薦對既有職務影響較低者；選定道具後則以結論、預期結果、風險、替代方案四段式顯示能力／忠誠損失、職務加權影響及可改索回的低影響道具。若沒有其他持有道具，替代方案會依武器、馬匹、書籍、寶物的對應能力建議本城較適合的非君主受賞者，或建議先保留於勢力庫存。建言只讀取資料，不會自動選擇、索回或賞賜道具。

33. **已完成—登用人士的建言、清除與面板警告**：登用人士面板在武將選擇列加入「清除選擇」，並於確認按鈕旁加入「登用建言」。未選登用對象時，建言會在可見且合法的候選中，依魅力、智力、政治、統率、武力、戰鬥的綜合評分推薦優先接觸者；已選對象時，建言會列出能力、目前所屬、基本登用金與已設定的金／糧／道具贈禮，並明示成功後加入本城及原有職務解除的結果。候選評分僅供比較，實際登用仍依既有出仕意願結算。清除選擇後，確認登用會在同一面板顯示「請選擇武將」警告，不只寫入 Gameplay Log；建言與按鈕均不會自動選人或送出登用。

34. **已完成—登用接受分數建言**：登用建言會以與 `ExecuteHireOfficer` 共用的接受分數與門檻，直接顯示目前贈金、贈糧與贈送道具下的「可成功」或「目前不足」，並列出接受分數／門檻及高出或尚差的分數。自由人士門檻為 80，受僱武將門檻為 40；分數會沿用既有的城市民心／君主魅力／目標魅力／野心，或受僱者的忠誠／野心，以及贈禮加成。這是既有確定性接受判定的預覽，不另行引入隨機百分比，故與確認登用的實際結算一致。

35. **已完成—登用的君主關係加成**：候選武將與玩家君主的關係會納入登用接受分數：血親 `+20`、非血親親族 `+10`、一般親族 `+8`，未知或無關係為 `0`。關係會雙向讀取角色資料，與既有繼位關係判斷一致；登用建言會同時顯示關係名稱與加成。此加成由接受分數共用函式提供，確認登用與建言使用相同結果。

36. **已完成—俘虜管理建言**：俘虜管理面板將「俘虜建言」置於招降、釋放、處決按鈕旁，並沿用既有肖像式參謀對話框。未選俘虜時，建言會依目前贈金、道具與任命職位，推薦招降成功率最高、再比較綜合能力的俘虜；選定後會顯示招降成功率、能力、忠誠、野心、目前贈禮與對應建議。成功率預估與招降結算共用同一公式，包含關係、君主魅力、忠誠、野心、城市民心、贈金、道具與任命職位，並保留既有 5% 至 95% 的隨機成功率範圍；建言不會自動處置任何俘虜。

37. **已完成—工兵與戰役裝備獨立配置**：攻擊與守城面板的武將配兵僅保留步、槍、騎、弓、弩五種兵種；工兵改為不需武將統率的「戰役支援」數量。補給車、衝車、雲梯、投石車亦改由此支援列配置，每方各限一輛／具；選擇任何裝備必須同時派出至少一隊工兵。確認命令時工兵與三種城市庫存攻城器會一併保留，月末建立戰役時工兵與每項設備會作為無武將的戰役隊伍加入；補給車提供既有補給車戰場能力。舊的武將統率攻城兵配置不再由 UI 產生。

38. **已完成—測試工具的戰役支援補給**：God Mode 的測試工具新增「戰役裝備 +1」與「工兵 +300」。兩者只對目前選取的玩家城市生效；前者將補給車、衝車、雲梯、投石車庫存各加一，後者直接加入 300 名工兵。補給車已具城市庫存，戰役支援列會檢查並在出征時保留一輛。操作會寫入 Gameplay Log 並刷新城市面板。

39. **已完成—城市檢視補給車欄位**：查看城市的城市表格在雲梯後新增「補給車」欄位。資訊可見時以與其他攻城器械一致的「庫存（建造進度／所需進度）」格式顯示，例如 `1 (0/100)`；敵方未取得完整情報時維持未知遮罩；欄位可點擊排序。

40. **已完成—補給車建造**：內政的「建設」項目在工坊達 Lv.1 後提供「補給車」。它與衝車、投石車、雲梯共用建造武將、每月投入與 100 點完成一輛的進度規則；完成後增加城市補給車庫存，供戰役支援列選用。

41. **已完成—戰役裝備工兵需求**：戰役支援依裝備加總所需工兵：補給車 `20`、衝車 `30`、雲梯 `50`、投石車 `50`。每項裝備各自佔一列，避免在窄面板內橫向捲動；城市沒有庫存的裝備不顯示，有庫存的裝備可直接點選。若當下配置的工兵不足，按鈕會保持未選取並在同一面板提示所需／已配置數量。確認攻擊或守城時會再次驗證，避免配置在資料變動後失效。

42. **已完成—工兵隊與裝備操作員分離**：戰役支援的工兵輸入改為「工兵隊」，而非包含裝備操作員的總數。每件已選裝備會先從城市工兵總數預留操作員；工兵隊可分配上限為城市總工兵扣除該預留數。例如 300 工兵選補給車後，補給車操作員占 20，工兵隊最多可配置 280；實際出征與城市扣除為工兵隊加上操作員總數。

43. **已完成—戰後戰役裝備返還**：補給車、衝車、雲梯與投石車在出征時自城市庫存保留。戰役結束、部隊撤退或未部署援軍返還時，仍存活的裝備隊會把該裝備與相應的工兵操作員一併返還至其目的城市；裝備隊在戰場被摧毀時，裝備與操作員才會損失。月末攻擊在建立戰役前取消、衝突或失敗的返還路徑亦會歸還補給車庫存。

44. **已完成—測試工具設施升級**：God Mode 的測試工具新增「弓坊升級」與「馬場升級」。按鈕只對目前選取的玩家城市生效，每次將相應設施提升一級並把該設施建造進度重設為 `0`；操作會寫入 Gameplay Log 並刷新城市資訊。

45. **已完成—城市移動兵種與軍資配置**：城市「移動」視窗的目標城市標題會使用 locale key；步兵、槍兵、騎兵、弓兵、弩兵及工兵會各自顯示「城內」可用數量，讓玩家逐項選擇移動量；補給車、衝車、雲梯與投石車亦各自顯示庫存。各列提供本地化的「最大」按鈕以一次填入上限；兵種或軍資的城內數量為 `0` 時整列不顯示。Gold、Food 與 Horse 亦有「最大」按鈕，且初始值均為 `0`。命令與月末結算會保留原選擇的兵種配置，不會以其他兵種替代補足；俘虜在檢視與移動名冊的忠誠欄均顯示 `-`。
