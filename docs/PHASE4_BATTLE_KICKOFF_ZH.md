...
4. **已完成—snapshot 與場景切換主幹**：第十個完整作戰日後將戰場 DTO 寫入 Campaign、返回 Gameplay，下一月可由「持續戰役」重載；Field → City 使用同一 Campaign 並清除不相容地圖 snapshot。
   ...
   - 戰役撤退目的地會列出可返回的來源城與戰役錨點相鄰的己方／無主城市；但守方已在目標城內時，目標城就是其來源城，不能把同一座城當作撤退目的地，僅能選相鄰合法城市。若沒有合法目的地，撤退不會執行。
...
14. **已完成—玩家戰役撤退目的地對話框**：玩家帶武將戰隊抵達撤離區後，會以 `RetreatDestinationDialog.tscn` 顯示有效的來源己方城市及相鄰己方／無主城市；來源城市永遠優先列出為「返回來源城：城市」，每個其餘候選都是明確的「撤往：城市」按鈕，而非不可辨識的文字。使者借用盟軍步兵時亦開啟此對話框，但候選依使者的原勢力判定；選定城市只接回使者，盟軍步兵另行返還盟軍來源城。此候選集合同時由 `BattleCampaignService` 驗證與執行，AI 也使用相同規則。對話框沿用主遊戲深色面板、金棕標題列與指令按鈕樣式，初次置中、可拖曳並攔截戰場輸入。沒有合法目的地時不開啟空白選單，直接顯示「沒有可前往的城市，無法撤退。」

15. **已完成—回合 banner 輸入鎖定**：`TurnBanner` 顯示期間，同步啟用全畫面透明輸入遮罩與戰場 `_Input`／`_UnhandledInput` 狀態門檻；玩家不能點選地圖、部隊、指令、Options 或拖曳面板。banner 由同一 serial 計時器隱藏時才解除鎖定，避免連續回合的舊 timer 提早恢復輸入。
