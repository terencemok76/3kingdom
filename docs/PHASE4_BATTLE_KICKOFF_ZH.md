...
- Gameplay 大地圖的浮動「軍報記錄」同樣提供「複製全部」按鈕；它會把完整時間順序日誌（不受目前 `本勢力／全部` 篩選影響）寫入系統剪貼簿，完成後按鈕顯示「已複製」。
...
4. **已完成—snapshot 與場景切換主幹**：第十個完整作戰日後將戰場 DTO 寫入 Campaign、返回 Gameplay，下一月可由「持續戰役」重載；Field → City 使用同一 Campaign 並清除不相容地圖 snapshot。
   ...
   - Option UI 提供持久化的「AI 決策偵錯」開關，預設關閉。開啟時只向 Gameplay 日誌追加每座 AI 城的評估起始狀態與最終執行結果；AI 實際選擇外交行動時，另列出外交類型、目標勢力與命中的門檻／兵力／關係值理由（停戰、毀約、索貢、同盟或送禮）。即使偵錯關閉，只要 AI 向玩家提出外交案，Gameplay 日誌仍會保留這條理由，讓玩家在接受或拒絕前能追溯提案動機；AI 對 AI 的理由則仍只在偵錯開啟時顯示。不修改 AI 評分、隨機性或行動結果。所有 Gameplay 日誌均由共用入口加上當前年月，例如「184年2月｜…」，避免月結、災害、移動、偵查與 AI 偵錯各自使用不一致的日期格式。
   - 偵查成功日誌會列出發動勢力君主與執行武將，例如「184年2月｜張角：管亥 偵查『濮陽』成功」；不再誤列被偵查城市的君主或行政官，且日期不會重複顯示。
...
12. **已完成—勢力結果提示**：Gameplay 偵測到原本持有城市的勢力失去全部城市時，會在既有日誌之外以同戰報風格的 modal 顯示勢力滅亡；多個勢力會依序顯示。玩家因失去全部城市或沒有存活的所屬武將而敗北時，顯示「遊戲敗北」modal；確認後隱藏地圖與 Gameplay HUD、恢復 Main Menu 與主選單音樂。若戰報仍在畫面上，結果提示會等戰報關閉後才顯示。
