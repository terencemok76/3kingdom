...
- `MoveRange` 同時是每個單位在其所屬行動方內累積可走的最大格數，不是每次點選 Move 的獨立上限。一般移動每跨一格扣除 `1` 點剩餘移動格數，嚴重受損橋格則依橋梁規則扣除 `3` 點；因此步兵的 `4/4` 在一般地先走兩格後會成為 `2/4`，即使行動力仍足夠也不能在同一行動方再走超過兩格。騎兵維持自己的較高 `MoveRange`（目前為 `6`），同樣受行動力與累積移動格數雙重限制。battle quick save/load 亦保存剩餘移動格數。
