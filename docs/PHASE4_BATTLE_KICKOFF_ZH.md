...
- 有武將的戰隊撤退時，原撤退武將仍先播放既有 `retreat` 台詞；其台詞結束後，若撤退位置 `6` 格內有可見敵方武將，則有 `55%` 機率由其中一人隨機播放本地化的 `enemy_retreat` 回應台詞。這不影響撤退結算，重要高優先台詞仍可覆蓋此回應。
...
- 玩家守城時，防禦配兵 UI 在「確認守城」之前提供相鄰城市的援軍入口。本國相鄰城切換至該城的武將／兵種／兵力編輯後加入請求；同盟相鄰城必須選擇一名未編入守軍的本城使者，再選擇請求規模。使者不另行花費外交移動日，但在請求被接受至戰役結束期間鎖定，並以其 Intelligence／Charm 加入同盟君主的即時接受判定；接受後盟友 AI 會在總請求規模內，自動決定借給使者統率的步兵數與由盟軍武將統率的其餘援軍。使者隊為玩家控制、步兵仍屬盟友來源城；盟軍武將隊維持盟軍 AI 控制。兩類隊伍都隨戰役建立進入 `Traveling`，直接鄰城從戰役第 `1` 日起計算 `2` 日 ETA，而不是等待月末；戰後步兵返還盟城，使者返回己方守城城市。
...
- 援軍只在己方時段開始前的 Dawn 進場。攻方只使用攻方外側起始部署區；守方援軍也只使用守方外側起始部署區，不能直接生成於內城。只有原本的城市守軍可在內城開始。入口受阻時進入等待隊列，不可重疊生成或直接出現在敵軍旁邊；中央事件提示與戰鬥紀錄會列出抵達援軍的勢力、武將／兵種／兵力及 Gold／Food。援軍實際部署後，戰鬥紀錄另以「原有 + 援軍 = 最終」列出該側 Gold／Food，且 top bar 立即同步最終值。若當前有攻守方回合 banner，援軍中央提示會等 banner 隱藏後才顯示，避免重疊；援軍中央提示關閉後，才建立各在場武將的士氣 `+/-` popup，並有 `65%` 機率由本批實際入場、具武將的援軍隨機一人說本地化到達台詞，確保效果可見且不為 Reserve／無武將隊伍觸發；同一 Dawn 的多筆援軍會依勢力合併為一則提示，避免重複標題堆疊。
