...
internal sealed class MilitaryUiContext : IFloatingOverlayContext
{
    ...
    public void ShowAdvisorMessage(OfficerData? advisor, string speakerRole, string advice) => _owner.MilitaryShowAdvisorMessage(advisor, speakerRole, advice);
    ...
    public CommandResult ExecuteMoveCommand(int targetCityId, TroopAllocationData troopAllocation, int gold, int food, int horses, SiegeEngineAllocationData siegeEngineAllocation, List<int> officerIds, List<int> captiveOfficerIds) => _owner.MilitaryExecuteMoveCommand(targetCityId, troopAllocation, gold, food, horses, siegeEngineAllocation, officerIds, captiveOfficerIds);
    public CommandResult ExecuteAttackCommand(int targetCityId, int troops, int gold, int food, List<AttackOfficerDeploymentData> deployments, BattleSupportDeploymentData battleSupport, List<int> officerIds, DefenderBattlePlan? defenderBattlePlanOverride = null) => _owner.MilitaryExecuteAttackCommand(targetCityId, troops, gold, food, deployments, battleSupport, officerIds, defenderBattlePlanOverride);
    public bool IsGodModeEnabled() => TurnManager?.World?.ViewAllInformationEnabled ?? false;
